class Automator {
  ...
  async runProfilesAutomation(app) {
    ...
    app.db.getProfilesQueueStatus = await app.profileController.getProfilesQueueStatus();

    await app.logger.info(`Initial Profiles Queue stats are as below:\n${app.db.getProfilesQueueStatus}`);
  ...
  async runTasksAutomation(app) {
    ...
    app.db.currentTasksQueueStatus = await app.taskController.getTasksQueueStatus();
    ...
    for (const task of app.db.currentTasksQueue) {
      ...
      console.log(`Running task: ${task} for profile: ${app.db.currentProfile.username}`);
